...
The output layer is then performing a weighted sum of the activations from the hidden layer, as in $$\hat{y} = \sum_{i=1}^m d_ig_i = \mathbf{d}^T\mathbf(g)$$, where $$d_i$$ are the weights.
Each edge in the network implies a parameter. The dangling input edges of the hidden nodes indicate the biases ($$b_i$$).
