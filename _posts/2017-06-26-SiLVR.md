...
Let us define a variable $$t$$ along this direction. $$t$$ can be considered as a hidden or _latent variable_. This latent variable is defined by the vector $$(x_1,x_2) = (1,2)$$ in the original input space. This means that it is more closely aligned to $$x_2$$ than to $$x_1$$ by a factor of 2. Since the dependent variable shows all its variation along this latent variable direction, we can infer that in some global sense $$x_2$$ has twice the influence on $$y$$ compared to the influence of $$x_1$$. In the extreme case, if the latent variable direction was defined by $$(x_1,x_2) = (0,2)$$, then $$x_1$$ would have no influence on $$y$$ at all.
