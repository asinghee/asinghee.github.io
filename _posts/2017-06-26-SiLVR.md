...
# The problem
Consider the standard regression problem, where we are given a sample set of vectors $$\mathbf{x}_i$$ from an input space and corresponding values $$y_i$$ of a dependent variable, and wish to generate a function $$\hat{y} = f(\mathbf{x})$$ such that the errors between $$\hat{y}_i = f(\mathbf{x}_i)$$ and $$y_i$$ are minimized in some sense.
