...
$$
L = \sum_{j=1}^N (y_j - g_{1,j})^2
$$

Later we will see that we use a slightly modified version of this loss to achieve more robust convergence.
...
![opamplvs]({{site.baseurl}}/images/Opamp_all_inputwts.tif "The weight vectors for the first latent variable direction.")

We can start to interpret these learnt model parameters in terms of the modeled system's behavior. For example, we see that `Gain`, `PM` and `Offset` have near identical weight vectors. This suggests that they are largely varying along the same direction. In fact, if we plot them against the first latent variable for `Gain`, we can see this in action:

![commonlv]({{site.baseurl}/images/Opamp_GainPMOffset_onGain_corrected.tif "`Gain`, `PM`, `Offset` plotted along the first latent variable discovered for `Gain`.)
...
Digging a little deeper into the weight vectors, we see a nice way to quantify this relationship. 
